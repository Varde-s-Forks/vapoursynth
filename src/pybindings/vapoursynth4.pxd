...
cdef extern from "include/VapourSynth4.h" nogil:
    ...
    ctypedef const VSAPI *(*VSGetVapourSynthAPI)(int version)

    ctypedef void (*VSPublicFunction)(const VSMap *input, VSMap *out, void *userData, VSCore *core, const VSAPI *vsapi)
    ctypedef void (*VSInitPlugin)(VSPlugin *plugin, const VSPLUGINAPI *vspapi)

    ctypedef void (*VSFreeFunctionData)(void *userData)
    ctypedef const VSFrame *(*VSFilterGetFrame)(int n, int activationReason, void *instanceData, void **frameData, VSFrameContext *frameCtx, VSCore *core, const VSAPI *vsapi)
    ctypedef void (*VSFilterFree)(void *instanceData, VSCore *core, const VSAPI *vsapi)

    ctypedef void (*VSFrameDoneCallback)(void *userData, const VSFrame *f, int n, VSNode *node, const char *errorMsg)
    ctypedef void (*VSLogHandler)(int msgType, const char *msg, void *userData)
    ctypedef void (*VSLogHandlerFree)(void *userData)
